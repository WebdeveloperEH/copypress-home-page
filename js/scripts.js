...
function copypress_storage_get(var_name) {
  "use strict";
  return copypress_isset(COPYPRESS_STORAGE[var_name])
    ? COPYPRESS_STORAGE[var_name]
    : "";
}
...
function copypress_storage_get_array(var_name, key) {
  "use strict";
  return copypress_isset(COPYPRESS_STORAGE[var_name][key])
    ? COPYPRESS_STORAGE[var_name][key]
    : "";
}

function copypress_storage_set_array(var_name, key, value) {
  "use strict";
  if (!copypress_isset(COPYPRESS_STORAGE[var_name]))
    COPYPRESS_STORAGE[var_name] = {};
...
function copypress_empty(obj) {
  ...
  return (
    ...
    (typeof obj == "string" && copypress_alltrim(obj) == "") ||
    obj === 0
  );
}
...
function copypress_color_picker_show(id, fld, func) {
  ...
  var def =
    fld.val().substr(0, 1) == "#"
      ? fld.val()
      : copypress_rgb2hex(fld.css("backgroundColor"));
...
function copypress_document_animate_to(id, callback) {
  ...
  var speed = Math.min(
    ...
    Math.max(
      300,
      Math.round((Math.abs(oft - st) / jQuery(window).height()) * 300)
    )
  );
...
if (window.dao === undefined)
  function dao(s, r) {
    if (COPYPRESS_STORAGE["user_logged_in"])
      alert(copypress_debug_object(s, false, r));
  }
...
function copypress_init_actions() {
  "use strict";
  if (
    COPYPRESS_STORAGE["vc_edit_mode"] &&
    jQuery(".vc_empty-placeholder").length == 0 &&
    COPYPRESS_STORAGE["theme_init_counter"]++ < 30
  ) {
...
function copypress_ready_actions() {
        ...
        src: [
          {
            ...
            type:
              "video/" +
              copypress_get_file_ext(COPYPRESS_STORAGE["background_video"]),
          },
        ],
        onLoad: function () {},
      });
  ...
  if (
    jQuery(".copypress_tabs:not(.inited)").length > 0 &&
    jQuery.ui &&
    jQuery.ui.tabs
  ) {
  ...
  jQuery(".copypress_tabs_ajax").on("tabsbeforeactivate", function (event, ui) {
    ...
    if (ui.newPanel.data("need-content"))
      copypress_tabs_ajax_content_loader(ui.newPanel, 1, ui.oldPanel);
  });
  ...
  if (rows.length > 0) {
    rows.each(function () {
      "use strict";
      if (
        !jQuery(this).next().hasClass("copypress_layouts_row_fixed_placeholder")
      )
        jQuery(this).after(
          '<div class="copypress_layouts_row_fixed_placeholder"></div>'
        );
    });
  ...
  if (jQuery(".sc_layouts_menu_nav").hasClass("inited"))
    jQuery(".sc_layouts_menu_nav_area").addClass("menu_show");
  jQuery("select:not(.esg-sorting-select)").wrap(
    ...
    if (panel.length == 0) {
      jQuery
        ...
        .done(function (response) {
          ...
          copypress_loadmore_add_items(
            jQuery(".content > .posts_container").eq(0),
            jQuery(response).find(
              '.content > .posts_container > article,.content > .posts_container > div[class*="column-"]'
            )
          );
        });
    } else {
      jQuery
        .post(COPYPRESS_STORAGE["ajax_url"], {
...
function copypress_fix_header(rows, resize) {
  ...
  rows.each(function () {
    ...
    var offset = parseInt(
      jQuery(this).hasClass("copypress_layouts_row_fixed_on")
    ...
    if (scroll_offset + rows_offset <= offset) {
      ...
      if (!jQuery(this).hasClass("copypress_layouts_row_fixed_on")) {
        if (rows_offset + h < jQuery(window).height() * 0.33) {
          ...
          jQuery(this)
            .addClass("copypress_layouts_row_fixed_on")
            .css({
              top: rows_offset + "px",
            });
...
function copypress_fix_sidebar() {
  ...
  if (sb.length > 0) {
    if (
      jQuery(".page_content_wrap .content_wrap .content").css("float") == "none"
    ) {
...
function copypress_init_media_elements(cont) {
  "use strict";
  if (
    COPYPRESS_STORAGE["use_mediaelements"] &&
    ...
    if (window.mejs) {
      ...
      cont.find("audio:not(.inited),video:not(.inited)").each(function () {
        ...
        if (
          ...
          (COPYPRESS_STORAGE["init_all_mediaelements"] ||
            (!jQuery(this).hasClass("tpl-audio-shortcode") &&
...
function copypress_tabs_ajax_content_loader(panel, page, oldPanel) {
  ...
  if (panel.html().replace(/\s/g, "") == "") {
    ...
    panel.html(
      '<div class="copypress_tab_holder" style="min-height:' +
        height +
        'px;"></div>'
    );
    ...
    .done(function (response) {
      ...
      if (rez.error !== "") {
        ...
        panel.prepend(rez.data).fadeIn(function () {
          ...
          setTimeout(function () {
            panel
              .find(".copypress_tab_holder,.copypress_tab_content_remove")
              .remove();
            jQuery(window).trigger("scroll");
          }, 600);
        });
...
function copypress_comments_validate(form) {
  ...
  var comments_args = {
    ...
    error_message_class:
      "copypress_messagebox copypress_messagebox_style_error",
...
jQuery(document).on("action.init_hidden_elements", function (e, cont) {
  ...
  if (
    ...
    COPYPRESS_STORAGE["button_hover"] != "default"
  ) {
    jQuery(
      '.theme_button:not([class*="sc_button_hover_"]), .more-link:not([class*="sc_button_hover_"]), .trx_addons_hover_content .trx_addons_hover_links a:not([class*="sc_button_hover_"]), .copypress_tabs .copypress_tabs_titles li a:not([class*="sc_button_hover_"]), .hover_shop_buttons .icons a:not([class*="sc_button_hover_style_"]), .woocommerce #respond input#submit:not([class*="sc_button_hover_"]), .woocommerce .button:not([class*="shop_cart"]):not([class*="sc_button_hover_"]), .woocommerce-page .button:not([class*="shop_cart"]):not([class*="sc_button_hover_"]), #buddypress a.button:not([class*="sc_button_hover_"]) '
    ).addClass(
    ...
    jQuery(
      '.post_item_single .post_content .post_meta .post_share .social_item a:not([class*="sc_button_hover_style_"]), .woocommerce #respond input#submit.alt:not([class*="sc_button_hover_style_"]), .woocommerce a.button.alt:not([class*="sc_button_hover_style_"]), .woocommerce button.button.alt:not([class*="sc_button_hover_style_"]), .woocommerce input.button.alt:not([class*="sc_button_hover_style_"]) '
    ...
    jQuery(
      '.sidebar .trx_addons_tabs .trx_addons_tabs_titles li a:not([class*="sc_button_hover_style_"]), .copypress_tabs .copypress_tabs_titles li a:not([class*="sc_button_hover_style_"]), .widget_tag_cloud a:not([class*="sc_button_hover_style_"]), .widget_product_tag_cloud a:not([class*="sc_button_hover_style_"]) '
    ...
    jQuery(
      ".mejs-controls button, .mfp-close, .sc_button_bg_image, .hover_shop_buttons a, .sc_layouts_row_type_narrow .sc_button "
    ).removeClass("sc_button_hover_" + COPYPRESS_STORAGE["button_hover"]);
  }
});
jQuery(document).on(
  "action.add_googlemap_styles",
  copypress_trx_addons_add_googlemap_styles
);
...
function copypress_trx_addons_init(e, container) {
  ...
  container
    ...
    .addClass("inited")
    .attr("data-color", COPYPRESS_STORAGE["alter_link_color"]);
}
