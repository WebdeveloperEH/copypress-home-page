...
function trx_addons_ready_actions(e, container) {
  ...
  if (TRX_ADDONS_STORAGE["popup_engine"] == "pretty") {
    ...
    container
      .find(".trx_addons_popup_link:not(.inited)")
      .addClass("inited")
      .magnificPopup({
        type: "inline",
        focus: "input",
  ...
  if (TRX_ADDONS_STORAGE["login_via_ajax"] && !error) {
    jQuery
      .post(TRX_ADDONS_STORAGE["ajax_url"], {
        ...
        nonce: TRX_ADDONS_STORAGE["ajax_nonce"],
        redirect_to:
          form.find("#redirect_to").length == 1
            ? form.find("#redirect_to").val()
            : "",
  ...
  if (rows.length > 0) {
    rows.each(function () {
      ...
      if (!jQuery(this).next().hasClass("sc_layouts_row_fixed_placeholder"))
        jQuery(this).after(
          '<div class="sc_layouts_row_fixed_placeholder"></div>'
        );
    });
...
jQuery(document).on("action.ready_trx_addons", function () {
  ...
  if (jQuery(".sc_layouts_cart").length > 0) {
    jQuery("body:not(.added_to_cart_inited)")
      .addClass("added_to_cart_inited")
...
jQuery(document).on("action.ready_trx_addons", function () {
  ...
  jQuery(".sc_layouts_menu").each(function () {
    ...
    if (jQuery(this).find(">ul.inited").length == 1)
      jQuery(this).addClass("menu_show");
  });
  jQuery(
...
function trx_addons_init_sliders(e, container) {
  ...
  if (container.find(".sc_slider_controller:not(.inited)").length > 0) {
    container.find(".sc_slider_controller:not(.inited)").each(function () {
      ...
      jQuery("#" + slider_id + " .slider_swiper").attr(
        "data-controller",
        controller_id
      );
      ...
      TRX_ADDONS_STORAGE["swipers"][id] = new Swiper("." + id, {
        ...
        onSlideChangeStart: function (swiper) {
          ...
          var controlled_slider = jQuery(
            "#" +
              slider.data(is_controller ? "controlled-slider" : "controller") +
              " .slider_swiper"
          );
        ...
        onSlideChangeEnd: function (swiper) {
          ...
          titles
            .eq(jQuery(swiper.slides[swiper.activeIndex]).data("slide-number"))
            .addClass("active")
            .fadeIn(300);
          cont
...
function trx_addons_resize_sliders(e, container) {
  ...
  container.find(".slider_swiper.inited").each(function () {
    ...
    if (
      !jQuery(this).hasClass("slider_noresize") ||
